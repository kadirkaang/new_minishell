...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/09 17:02:55 by kgoc              #+#    #+#             */
/*   Updated: 2023/10/09 17:03:49 by kgoc             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_env_to_lexlist(t_lexer **lex_list)
{
	...
	tokens = (*lex_list)->lex; // tokens
	while (tokens)
	{
		i = 0;
		tmp2 = ft_strdup(tokens->content);
		tmp = quote_control(tmp2);
		while (tmp[i])
		{
			_get_env_to_lexlist(&tmp, i, &tokens);
			if (tmp[i] == '$' && tmp[0] != '\'')
			{
				check_env(tokens, tmp, i);
